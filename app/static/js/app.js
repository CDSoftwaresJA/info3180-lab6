...
const NewsList = Vue.component('news-list',{
    ...
    <p>You are searching for {{ searchTerm }}</p>     </div> </div>
    <button class="btn btn-primary mb-2" @click="searchNews">Search</button>
            <div v-for="article in articles" class="news__item">
            <b>{{ article.title }}</b>
            <br>
            <img v-bind:src="article.urlToImage" height=200> 
            <br>
            {{ article.description }}
            </div>   
    </div>
    ...
    data: function(){
        ...
        methods: {         
            searchNews: function() { 
                let self = this;         
                fetch('https://newsapi.org/v2/top-headlines?q='+ self.searchTerm + '&language=en&apiKey=')           
                .then(function(response) {             
                    return response.json();          
                    })           
                .then(function(data) {             
